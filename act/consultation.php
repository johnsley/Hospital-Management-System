 ...
 <div id="wrapper">
        ...
        <div id="page-wrapper">
           ...
           <div class="row">
				...
				<div class = "panel-body">
					<form id = "form_dental" method = "POST" enctype = "multipart/form-data">
						<div class = "col-sm-12" style="padding:0px;padding-top: 1%">
							...
							?>
							<div class="col-sm-6">
							<input name = "category" type = "hidden"  value = "consultation">
			...
			<a id = "show_itr">&nbsp;<img src="images/patient.jpg" style="width: 50px;height: 50px"> <strong>Add Patient Consultation</strong></a>
			</div>
			<div  id = "a_table" class="panel panel-default">
				<div class = "panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<tbody>
						<?php
	                        $query = $conn->query("SELECT * FROM `service` WHERE `category` = 'consultation'  ORDER BY `date` DESC") or die(mysqli_error());
	                        while($fetch = $query->fetch_array()){
	                            ...
	                            $t11 = $t1->fetch_array(); 
	                            $tquery = $conn->query("SELECT * FROM `triage` WHERE `service_id`='$service_id' ") or die(mysqli_error());
	                            $tfetch = $tquery->num_rows;          
	                         ?>
							<tr>
								<?php
                                if($fetch['reversal'] == 'R'){
                                ...
                                <?php
                                 if($tfetch > 0){
                                 ?>
							    <td><center><a href = "triage.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default" style="color:red;border:2px solid red;font-size: 0.9em">DX/TX</a></center></td>
							    <?php
							    }else{
							    	?>
							    	<td><center><a href = "triage.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default">DX/TX</a></center></td>
							    	<?php
							    }
							    }else{
                                ?>
                                ...
                                ?>
                                <a href = "edit_consultation.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default" title="Adjust">ADJUST</a>
                                <?php
                                }
								?>
							    </center></td>
							    <?php
                                 if($tfetch > 0){
                                 ?>
							    <td style="color:red"><center><a href = "triage.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default"  style="color:red;border:2px solid red;font-size: 0.9em">DX/TX</a></center></td>
							    <?php
							      }else{
							      	?>
							      	<td><center><a href = "triage.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default">DX/TX</a></center></td>
