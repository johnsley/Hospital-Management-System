 ...
 <div id="wrapper">
        ...
        <div id="page-wrapper">
           ...
           <div class="row">
					    ...
					    <div class="col-sm-12" style="padding:0px;padding-top: 2%">
					    	<div class="col-sm-5">
								...
								<select id="item" name = "item" required = "required" style="width:100%;padding: 5px">
		                            ...
		                            <?php
										$query = $conn->query("SELECT * FROM `item` WHERE `type`='laboratory' ") or die(mysqli_error());
										while($row = $query->fetch_array()){
										$item_id = $row['item_id'];
										?>
			...
			<a id = "show_itr">&nbsp;<img src="images/patient.jpg" style="width: 50px;height: 50px"> <strong>Add Lab Test Charges</strong></a>
			</div>
			<div  id = "a_table" class="panel panel-default">
				<div class = "panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<tbody>
						<?php
	                        $query = $conn->query("SELECT * FROM `service` WHERE `category` = 'laboratory' ORDER BY `date` DESC") or die(mysqli_error());
	                        while($fetch = $query->fetch_array()){
	                            ...
	                            $t11 = $t1->fetch_array();
	                            $xquery = $conn->query("SELECT * FROM `labreps` WHERE `service_id`='$service_id' ") or die(mysqli_error());
							...
							<tr>
                                ...
                                <?php
                                 if($xfetch > 0){
                                  ?>
                                   <a href = "edit_lab.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default" style="color:red;border:2px solid red;font-size: 0.9em">EDIT</a>
                                  <?php
                                 }else{
                                   ?>
                                  <a href = "edit_lab.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default" title="Adjust">EDIT</a>
                                  <?php
                                 }
                                 ?>
                                <a href = "labreport.php?id=<?php echo $fetch['service_id']?>" class = "btn btn-sm btn-default">REPORT</a>
							    </center></td>
                                <?php
                                }
                                ?>
