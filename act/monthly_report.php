           ...
           <div class="col-lg-12">
			<div  id = "a_table" class="panel panel-default">
				<div class = "panel-heading">
					<label>MONTHLY REPORT - FROM: <span style="color:green"><?php echo $_POST['mindate'];?></span> TO: <span style="color:green"><?php echo $_POST['maxdate'];?></span></Label>
				...
				<div class = "panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<?php
	                        ...
	                        $query = $conn->query("SELECT * FROM `service` WHERE `date` >= '$mindate' AND `date` < '$maxdate' ORDER BY `date` DESC ") or die(mysqli_error());

	                        $c1 = $query->num_rows;
							
							$cash = $conn->query("SELECT SUM(paid) as total FROM `service` WHERE `date` >= '$mindate' AND `date` < '$maxdate' AND `payment` = 'Cash' ") or die(mysqli_error());
							$fcash = $cash->fetch_array();
							if($fcash['total'] > 0){$fcash = $fcash['total'];}else{$fcash = 0;}
							
							$mpesa = $conn->query("SELECT SUM(paid) as total FROM `service` WHERE `date` >= '$mindate' AND `date` < '$maxdate' AND `payment` = 'mpesa' ") or die(mysqli_error());
	                        ...
	                        while($fetch = $query->fetch_array()){
	                        	...
	                        	$q = $conn->query("SELECT * FROM `service_backup` WHERE `service_id` = '$service_id' AND `date` >= '$min2' AND `date` < '$max2' ORDER BY id LIMIT 1 ") or die(mysqli_error());
	                        	$c = $q->num_rows;
	                        	 if($c > 0){
	                                $f = $q->fetch_array();
	                            ...
	                            $outcash = $conn->query("SELECT SUM(price) as total FROM `outpatient` WHERE `date` >= '$mindate' AND `date` < '$maxdate' AND `payment`='cash' ") or die(mysqli_error());
		                        $foutcash = $outcash->fetch_array();
	                            if($foutcash['total'] > 0){$foutcash = $foutcash['total'];}else{$foutcash = 0;}

	                            $outmpesa = $conn->query("SELECT SUM(price) as total FROM `outpatient` WHERE `date` >= '$mindate' AND `date` < '$maxdate' AND `payment`='mpesa' ") or die(mysqli_error());
		                        $foutmpesa = $outmpesa->fetch_array();
	                            if($foutmpesa['total'] > 0){$foutmpesa = $foutmpesa['total'];}else{$foutmpesa = 0;}

		                        $pay = $conn->query("SELECT SUM(amount) as total FROM `payment` WHERE `date` >= '$mindate' AND `date` < '$maxdate' ") or die(mysqli_error());
	                    	...
	                    	?>
	                         <div class="col-sm-12" style="padding: 0px">
								...
								<div class="col-sm-12"  style="padding: 0px">
				                    ...
				                    <label>Total Monthly <span style="font-size: 1.5em">Cash</span> Collected</label>
									</div>
									<div class="col-sm-2">
										<?php 
								...
								<div class="col-sm-12"  style="padding: 0px">
									<div class="col-sm-4"  style="padding: 0px">
										<label>Total Monthly <span style="font-size: 1.5em">Mpesa</span> Collected</label>
									</div>
									<div class="col-sm-2">
										<?php 
								...
								<div class="col-sm-12"  style="padding: 0px">
									<div class="col-sm-4"  style="padding: 0px">
										<label>Total <span style="font-size: 1.5em">Monthly</span> Amount Collected</label>
									</div>
									<div class="col-sm-2">
										<?php 
