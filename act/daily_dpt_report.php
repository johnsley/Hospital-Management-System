           ...
           <div class="col-lg-12">
			<div  id = "a_table" class="panel panel-default">
				<div class = "panel-heading">
				    <label>DAILY <?php echo $_POST['dpt']?> REPORT</Label>
				...
				<div class = "panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<tbody>
						<?php
							$fromdate = $_POST['dailydate']; 
							$dpt = $_POST['dpt'];
							$date = date_create($_POST['dailydate']);
	                        ...
	                        $query = $conn->query("SELECT * FROM `service` WHERE `date` LIKE '%{$fromdate}%' AND `category` = '$dpt' ORDER BY `date` DESC ") or die(mysqli_error());
	                        $c1 = $query->num_rows;
							
							$cash = $conn->query("SELECT SUM(paid) as total FROM `service` WHERE `date` LIKE '%{$fromdate}%' AND `payment`='Cash' AND `category` = '$dpt' ") or die(mysqli_error());
							$fcash = $cash->fetch_array();
							if($fcash['total'] > 0){$fcash = $fcash['total'];}else{$fcash = 0;}
							
							$mpesa = $conn->query("SELECT SUM(paid) as total FROM `service` WHERE `date` LIKE '%{$fromdate}%' AND `payment`='mpesa' AND `category` = '$dpt' ") or die(mysqli_error());
	                        ...
	                        while($fetch = $query->fetch_array()){
	                        	...
	                        	$item_id = $fetch['item_id'];
	                        	 ...
	                        	 $q = $conn->query("SELECT * FROM `service_backup` WHERE `service_id` = '$service_id' AND `date` LIKE '%{$date2}%' AND `category` = '$dpt' ORDER BY id LIMIT 1 ") or die(mysqli_error());
	                        	 $c = $q->num_rows;
	                        	 if($c > 0){
	                                $f = $q->fetch_array();
	                            ...
	                            $outcash = $conn->query("SELECT SUM(price) as total FROM `outpatient` WHERE `date` LIKE '%{$fromdate}%' AND `payment`='cash' AND `category` = '$dpt' ") or die(mysqli_error());
		                        $foutcash = $outcash->fetch_array();
	                            if($foutcash['total'] > 0){$foutcash = $foutcash['total'];}else{$foutcash = 0;}

	                            $outmpesa = $conn->query("SELECT SUM(price) as total FROM `outpatient` WHERE `date` LIKE '%{$fromdate}%' AND `payment`='mpesa' AND `category` = '$dpt' ") or die(mysqli_error());
	                    ...
	                    if($c1 > 0){
	                    	?>
	                         <div class="col-sm-12" style="padding: 0px">
								...
								<div class="col-sm-12" style="padding: 0px">
									<div class="col-sm-4"  style="padding: 0px">
										<label>Dpt Cash Amount </label>
									</div>
									<div class="col-sm-2">
										<?php $fcashtotal = $fcash + $foutcash; echo "KES ".$fcashtotal?>
									</div>
								</div>
								<div class="col-sm-12" style="padding: 0px">
									<div class="col-sm-4"  style="padding: 0px">
										<label>Dpt Mpesa Amount </label>
									</div>
									<div class="col-sm-2">
										<?php $fmpesatotal = $fmpesa + $foutmpesa; echo "KES ".$fmpesatotal?>
									</div>
								</div>
								<div class="col-sm-12" style="padding: 0px"><hr></div>
								<div class="col-sm-12"  style="padding: 0px">
									<div class="col-sm-4"  style="padding: 0px">
				                    <label>Dpt Daily <span style="font-size: 1.5em">Amount</span> Collected</label>
									</div>
									<div class="col-sm-2">
										<?php 
										$cashsum = $fcashtotal + $fmpesatotal;
										echo "<strong>KES <span style='font-size:1.5em'>".$cashsum."</span></strong>";
										?>
									</div>
								</div>
						    </div>
