...
<br />
	<div id="print">
		...
		<center>	
			...
			<center><h4 style="margin: 5px">LAB REPORT</h4></center>

			<?php
				$query = $conn->query("SELECT * FROM `labreps` WHERE `service_id`='$_GET[id]' ") or die(mysqli_error());
			...
			<center>
				...
				<th>Matter</th> <th>Result</th> <th>Limits</th> <th>Class</th> <th>State</th>
					...
					<?php
                    $quer = $conn->query("SELECT * FROM `labreps` WHERE `service_id`='$_GET[id]' ") or die(mysqli_error());
				    while($fetc = $quer->fetch_array()){
				    	?>
				    	<tr>
                        <td>
						<?php echo $fetc['matter']; ?>
					    </td>
					     <td>
						<?php echo $fetc['result']; ?>
					    </td>
					     <td>
						<?php echo $fetc['limits']; ?>
					    </td>
					    <td>
						<?php echo $fetc['class']; ?>
					    </td>
					...
					?>
					<tr>
						<td colspan="5"><?php echo $fetch['comment'];?></td>
					</tr>
				</table>
			</center>
            <br>
